...
import ItvDialog from "../Dialogs/ItvDialog";
    ...
    <Box className={classes.component}>
      {data.map((temp) => (
        <Box textAlign="center" className={classes.wrapper}>
          ...
          <Button variant="contained" onClick={() => openItvDialog()}>
            Update
          </Button>
          <ItvDialog open={open} setOpen={setOpen} />
        </Box>
      ))}
    </Box>
  );
...
const ItvSlide = (props) => {
  return <>{props.multi === true ? <MultiSlide {...props} /> : ""}</>;
};

export default ItvSlide;
